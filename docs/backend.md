...
Socket.IO contract, security/visibility model, and test coverage. The frontend has not been
started yet — this covers `packages/shared` and `packages/server` only.
...
- **`src/gameLogic.ts`** — pure, RNG-injectable game rules:
  ...
  - `resolveGuess(state, cardId, guessesRemainingBeforeGuess)` — the core turn-resolution
    ...
    and winner/winReason if the guess ended the game. Used by `Room.revealCard` but fully
...
- **`defaultRoom.ts`** — `ensureDefaultRoom()` = `getOrCreateRoom(config.defaultRoomCode)`. This is
  the *only* place that hardcodes the single-room MVP assumption; the store and `Room` class know
...
- **`Room.ts`** — the stateful core. Holds `state: GameState` and `playerSockets: Map<playerId,
  ...
  - `disconnectSocket(playerId, socketId)` — guarded version of the above: only marks
    ...
    fires *after* the same player has already reconnected on a new socket (e.g. page refresh),
  ...
  - `submitClue(playerId, word, number)` — validates: game in progress, player is the active
    ...
    number.
  - `revealCard(playerId, cardId)` — validates: game in progress, not a spectator, player's team
    is the active team, captains cannot reveal cards themselves, a clue is active, the card
    exists and isn't already revealed. Delegates outcome resolution to `resolveGuess` and applies
    the result (cards, team counts, turn, clue, winner/winReason/phase).
  - `endTurn(playerId)` — validates: game in progress, not a spectator, player's team is active,
    a clue is active to pass on. Switches `turn` and clears `currentClue`.
...
- **`types.ts`** — `SocketData { playerId?, roomCode? }`; `AckCallback<T>`; the full
  ...
  `start_game`, `submit_clue`, `reveal_card`, `end_turn`, `leave_game`, `request_state`), each
  with a typed payload and ack signature; `ServerToClientEvents` (`game_state_update`, `error`,
  `player_joined`, `player_left`, `game_over`); `InterServerEvents = Record<string, never>`.
...
- **`handlers/gameHandlers.ts`** — `submit_clue`, `reveal_card`, `end_turn`. `reveal_card`
  additionally checks the (now-updated) `room.state` after a successful mutation and, if the
  guess ended the game, emits a non-authoritative `game_over` event (`{ winner, reason }`) to the
  whole room as UX sugar — `game_state_update` remains the source of truth.
...
| `reveal_card` | `{ cardId }` | Active team's non-captain only |
| `end_turn` | — | Active team only, requires an active clue |
...
- **`packages/server/src/__tests__/e2e/`** — real HTTP + Socket.IO server spun up on a random
  ...
  - `fullFlow.test.ts` (1 test) — full lobby → start → sequential reveal of all 9 starting-team
    cards across turn switches → win by `all_words_found`; asserts per-role visibility at each
    stage, including full-board reveal to everyone once the game finishes.
  - `assassin.test.ts` (1 test) — revealing the assassin card ends the game immediately with
    `winReason: "assassin_revealed"` and the revealing team losing.
  - `forbiddenActions.test.ts` (5 tests) — `CANNOT_START` on an unready lobby; `FORBIDDEN_ROLE`
    for a spectator on every mutating action; `NOT_CAPTAIN` for a non-captain and for the
    inactive team's captain; `NO_ACTIVE_CLUE` / `NOT_YOUR_TURN` on reveal attempts; `FORBIDDEN_ROLE`
    when a captain tries to reveal a card themselves.
